...
export const getConversationThreads = async (
  ...
  try {
    ...
    // Obtener mensajes de la colección messages (para otras plataformas)
    console.log('[getConversationThreads] Buscando mensajes en colección global messages...');
    ...
    console.log('[getConversationThreads] Mensajes encontrados en colección global:', messagesSnapshot.size);
    
    // Agregar mensajes de la colección global
    const globalMessages = messagesSnapshot.docs.map(doc => ({
      id: doc.id,
      ...doc.data()
    } as Message));
    
    allMessages = [...allMessages, ...globalMessages];
    
    console.log('[getConversationThreads] Total de mensajes encontrados:', allMessages.length);
